...
router.post('/signin', async (req, res) => {
  const {email, password} = req.body; // 1. req.body에서 데이터 가져오기
   ...
   if(!email || !password) {
    ...
    //5. status: 200 ,message: SIGNIN SUCCESS, data: id 반환
...
router.get('/', async (req, res) => {
  try{
    ...
    console.error(error);
    return res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, responseMessage.USER_READ_ALL_SUCCESS));
  }
})
