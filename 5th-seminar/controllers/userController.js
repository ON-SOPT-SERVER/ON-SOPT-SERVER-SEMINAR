...
const signup = async (req, res) => {
  ...
  try {
    ...
    const alreadyEmail = await userService.isValidEmail(email);
    ...
    //5. status: 200 message: SING_UP_SUCCESS, data: id, email, userName 반환! (비밀번호, salt 반환 금지!!)
...
const readOne = async (req, res, next) => {
  ...
  try{
    const user = await userService.getPostByUser(id);
